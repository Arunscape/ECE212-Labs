...
\textbf{FROM THE FIRST LAB  R E P L A C E T H I S!}
  ...
  pairing up the nth term of the first array with the nth term of the second, 
  creating an (x,y) coordinate pair. This represents the nth point on the graph. 
  
  These can be used alongside the formula for the area under a trapezoid, which is 
  $h\cdot(\frac{a+b}{2})$. In this formula it is assuming a trapezoid with parallel 
  ...
  will be rotating the system 90 degrees, and the values of a and b will instead be 
  two different y coordinates on the graph, and h will be the absolute value of the 
  difference between their respective x coordinates. By assuming that the x coordinate 
  ...
  the new equation is $\frac{(x1-x0)\cdot(y0+y1)}{2}$. Using this formula it will be 
  ...
  therefore this will demonstrate an understanding of the use of Register Indirect methods. 
