...
move.l (%a2)+, %d3 /* Move the value at the location pointed at by a2 to d3, then 
                      increment the address in a2 by the size of a long word (4) */
add.l (%a3)+, %d3  /* Add the value at the location pointed at by a3, then increment 
                      the address in a3 by the size of a long word            */
move.l %d3, (%a1)+ /* Move the sum to the location pointed at by a1, then increment 
                      the address in a1 by the size of a long word             */

add.l #1, %d2        /* Increment counter by 1 */
cmp.l 0x2300000, %d2 /* Compare the counter to the number of elements */
bge end              /* If the counter is greater than or equal to the number of elements, 
                        quit the loop and continue to the next part */
